...
@section('content')
    <h1 class="mb-4">Worker update</h1>
    ...
    <form method="post" action="{{ route('worker.update', $worker) }}">
        ...
        <div class="mb-3">
            ...
            <input name="surname" type="text" class="form-control" id="surname" placeholder="Working surname"
                   value="{{ $worker->surname ?? old('surname') }}">
            @error('surname')
            <p style="color: red">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea name="description" class="form-control" id="description" rows="3"
                      placeholder="Description">{{ $worker->description ?? old('description') }}</textarea>
            @error('description')
            <p style="color: red">{{ $message }}</p>
            @enderror
        </div>
        <div class="mb-3 form-check">
            <input name="is_married" type="checkbox" class="form-check-input"
                   id="is_married" {{ $worker->is_married ?? old('is_married') ? 'checked' : '' }}>
